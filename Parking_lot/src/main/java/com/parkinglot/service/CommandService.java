	...
	public static  CommandService getInstance(){
	   ...
	   private interface Command {
	    ...
	    private class Leave implements Command {
	        ...
	        public void validate() {
	            if (commandStringArray.length != 3) {
	                throw new IllegalArgumentException("leave command should have exactly 2 argument");
	            }
	        }

	        public String execute() {
	            TicketService ticketService = TicketService.getInstance();
	           double ticketcharg= ticketService.exitVehicle(Integer.parseInt(commandStringArray[2]));
	            return "Registration number "+commandStringArray[1]+" With Slot number " + commandStringArray[2] + 
	            		" is free with Charge:"+ticketcharg;
	        }
	    }
